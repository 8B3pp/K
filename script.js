...
function pokazLoaderMenu() {
    ...
    ['pasek-gorny-menu', 'pasek-zakladek', 'sekcja-strona-glowna', 'sekcja-ranking'].forEach(id => {
...
async function odswiezListeOgloszen() {
    ...
    lista.innerHTML = data.map(o => {
        const tytulEsc = o.tytul.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        const opisEsc = o.opis.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        return `<div style="background:${KOLORY.karta};padding:10px;border-radius:8px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;gap:10px;">
        ...
        <div style="display:flex;gap:6px;">
            <button onclick="window._edytujOgloszenie(${o.id}, '${tytulEsc}', '${opisEsc}', ${!!o.tylko_raz})" style="padding:6px 10px;background:#ff9800;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">Edytuj</button>
...
async function odswiezRankingSekcje() {
    ...
    const karty = await Promise.all(rankingi.map(async (r) => {
        ...
        let akcjaHtml = '';
        if (jestemCzlonkiem) {
            akcjaHtml = `<button onclick="window.opuscRanking(${r.id})" style="flex:1;padding:9px;background:#dc3545;color:white;border:none;border-radius:6px;font-weight:bold;cursor:pointer;">Opuść</button>`;
...
window.opuscRanking = async function(rankingId) {
    if (!confirm('Na pewno chcesz opuścić ten ranking? Stracisz swoje punkty.')) return;
    await sb.from('ranking_uczestnicy').delete().eq('ranking_id', rankingId).eq('user_id', mojaSesja.id);
    odswiezRankingSekcje();
    odswiezOdznakePunktow();
};
...
async function renderujSzczegolyRankingu(rankingId) {
    ...
    if (mojWpis && (mojWpis.ocena === null || mojWpis.ocena === undefined) && !bramka) {
