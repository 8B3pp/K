...
function wstrzykniStyleLadowania() {
    ...
    style.textContent = `
        ...
        #t7b-loader-menu { position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:#162447; border-radius:16px; padding:30px 26px; text-align:center; transition:opacity .4s ease; z-index:400; width:240px; box-shadow:0 20px 40px rgba(0,0,0,0.4); }
...
window.otworzPanelRankingu = async function() {
    ...
    modal.innerHTML = `<div style="background:${KOLORY.panel};width:100%;max-width:520px;margin:20px;padding:25px;border-radius:16px;max-height:85vh;overflow-y:auto;">
        ...
        <div style="margin:10px 0;"><label style="${STYL_LABEL}">Powiąż z wydarzeniem oznaczonym w terminarzu jako „liczy się do rankingu” (opcjonalnie)</label><select id="rank-wpis-terminarz" style="${STYL_INPUT}"><option value="">-- brak --</option>${opcjeHtml}</select><p style="font-size:11px;color:${KOLORY.tekstPrzygaszony};margin-top:6px;">Tu pojawią się tylko wydarzenia, przy których zaznaczono „Liczy się do aktywnego rankingu” podczas dodawania ich do terminarza.</p></div>
